...
labelRowNum = Label(root,text="Row Number",font=('Courier 10 bold'))
labelRowNum.grid(column = 15,row=1)

rowText = Text(root,height=1,width=5)
rowText.grid(column=17,row=1)

rowOffSetLabel = Label(root,text="Row offset",font=('Courier 10 bold'))
rowOffSetLabel.grid(column = 20,row=4)


rowOffSetText = Text(root,height=1,width=5)
rowOffSetText.grid(column=25,row=4)
##
# open button
open_button = ttk.Button(root,text='Open Files',command=select_files)
#open_button.place(x=10,y=450)
open_button.grid(column=10,row=250)
